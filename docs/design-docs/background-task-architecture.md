...
- 输出文件：`{workspace}/.nervure/sessions/<session_id>/background-tasks/<task_id>.output`（`BackgroundTaskState.output_file` 存相对 workspace 路径）。已有 `.onecode/sessions/` 只作为 legacy session 的读取兼容路径。
