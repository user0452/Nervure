...
本文描述 `services/memory/` 的架构边界：跨会话的长期记忆（long-term memory）、分层指令记忆（NERVURE.md）、运行时相关记忆注入和后台记忆提取。它区别于 `services/compaction/` 中只服务单会话连续性的 session memory（见 `compaction-architecture.md`）。

## 文件职责

| 文件 | 职责 |
|:---|:---|
| `auto_store.py` | `LongTermMemoryStore`：`.nervure/memory/` 目录读写与索引重建（兼容旧 `.onecode/memory/`） |
...
| `instruction_loader.py` | `InstructionMemoryLoader`：分层加载 `NERVURE.md`、rules、local overrides |
...
存储结构：`<workspace>/.nervure/memory/`，含索引 `MEMORY.md` 和各 `<topic>.md`（frontmatter：`name`、`description`、`type`）。已有 workspace 的 `.onecode/memory/` 可继续读取和受限写入。`MemoryKind`：`user`/`feedback`/`project`/`reference`。
...
`load()` 返回 `InstructionMemoryResult(files, rendered_text, fingerprint, warnings)`。加载顺序：user 层（`~/.nervure/NERVURE.md` + `~/.nervure/rules/*.md`，兼容旧 `~/.onecode/`）→ project 层（workspace 到 cwd 链上的 `NERVURE.md`、`.nervure/NERVURE.md`、`.nervure/rules/*.md`，兼容旧 `.onecode/`）→ local 层（`NERVURE.local.md`）。支持 `@./` include（最大深度 5），frontmatter `paths` glob 过滤。为兼容已有 workspace，对应 `NERVURE.md` / `NERVURE.local.md` 不存在时会回退读取同层的 `ONECODE.md` / `ONECODE.local.md`。
...
flowchart TD
  ...
  subgraph Write["写入（turn 结束）"]
    ...
    Child --> Files["编辑 .nervure/memory/*.md"]
  end
```
...
- **Layer 1（system prompt）**：`InstructionMemoryLoader` 渲染 NERVURE.md 规则，`LongTermMemoryPromptProvider` 提供记忆目录、类型规则和 `MEMORY.md` 截断索引；两者通过 `prompt-architecture.md` 中的 section 注入，各自带 fingerprint 支持缓存。
...
写入通过 `TurnStopped` hook（CLI 注册）启动 `dream` 后台任务，由 fork child 在后台更新 `.nervure/memory/`，不阻塞 turn 结束、不产生用户可见通知（见 `background-task-architecture.md`）。child 被标记 `long_term_memory_extraction_agent`，权限层只允许 `read_file`/`grep`/`glob` 读 + `write_file`/`edit_file` 写选定的 `.nervure/memory/`（或 legacy `.onecode/memory/`）下的 `.md` 文件（见 `permission-architecture.md`）。
...
- 长期记忆：`<workspace>/.nervure/memory/MEMORY.md` + topic files（兼容旧 `.onecode/memory/`）
- 指令记忆：`NERVURE.md`、`.nervure/NERVURE.md`、`.nervure/rules/*.md`、`NERVURE.local.md`（user/project/local 分层；兼容旧 `.onecode/`）
