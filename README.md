...
### 可中断、可恢复的状态 
运行时的对话记录、工具调用历史和轻量验证元数据被独立持久化。`/resume` 始终可以恢复 transcript，但只有 workspace、Git HEAD、已触及文件哈希、项目指令、工具/模型配置和权限模式都匹配时才标记为 `SAFE_RESUME`；配置变化标记为 `STALE_CONTEXT`，工作区变化标记为 `WORKSPACE_DIVERGED` 并清空旧文件缓存，要求写入前重新读取。文件修改工具会在执行前创建 checkpoint，`/undo` 恢复当前 session 的最新有效 checkpoint。
