...
class Repository:
    ...
    def _work_snapshot(self):
        result = {}
        for path in self.worktree.files():
            digest = hash_bytes(self.worktree.read(path), self.metrics)
            data = self.worktree.read(path)
            hash_bytes(data, self.metrics)
            result[path] = (data, digest)
        return result
    ...
    def status(self):
        ...
        by_hash = {snap[p][1]: p for p in untracked}
        for old in list(deleted):
            new = by_hash.get(tracked_hash[old])
            if new:
                result.renamed.append((old, new))
                deleted.remove(old)
                untracked.remove(new)
