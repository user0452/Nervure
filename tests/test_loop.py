...
async def _drain_continuation(loop: AgentLoop) -> None:
    async for _event in loop.continue_stream():
        pass
...
def test_top_level_prompt_drives_and_refreshes_tool_discovery(
    ...
    def descriptor(name: str, description: str) -> ToolDescriptor:
        def handler(_tool_input, runtime):
            return ToolExecutionResult(runtime.tool_call_id, name, "ok")
    ...
    model = FakeModelClient(
        [
            LLMResponse(assistant_message=assistant_message("one"), final_text="one"),
            LLMResponse(assistant_message=assistant_message("two"), final_text="two"),
            LLMResponse(
                assistant_message=assistant_message("continued"),
    ...
    run_to_final_text(loop, "run a shell command")
    run_to_final_text(loop, "unrelated quantum phrase")
